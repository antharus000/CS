...
class FirebaseService {
    ...
    // Add Stamp (iteration 2)
    // Increments coffeeCount by 1. If the new count reaches 10 it resets to 0
    // and increments rewardBalance by 1 (test 2.3).
    // Returns a user-facing confirmation string for the admin UI.
    func addStamp(accountID: String) async throws -> String {
        let doc = try await db.collection("rewards").document(accountID).getDocument()
        guard let data = doc.data() else { throw NSError(domain: "rewards", code: 404) }

        let current = data["coffeeCount"] as? Int ?? 0
        let balance = data["rewardBalance"] as? Int ?? 0

        if current + 1 >= 10 {
            // Reward triggered: reset count, increment balance
            try await db.collection("rewards").document(accountID).updateData([
                "coffeeCount":   0,
                "rewardBalance": balance + 1
            ])
            return "10th coffee! Free reward added. ☕"
        } else {
            try await db.collection("rewards").document(accountID).updateData([
                "coffeeCount": current + 1
            ])
            return "Stamp added. \(current + 1)/10 coffees."
        }
    }

    // Redeem Reward (iteration 2)
    // Decrements rewardBalance by 1. Guards against going below 0.
    func redeemReward(accountID: String) async throws {
        ...
        guard balance > 0 else {
            throw NSError(domain: "rewards", code: 400,
                          userInfo: [NSLocalizedDescriptionKey: "No rewards to redeem."])
        }
